...
def load_model(model_folder, trainer, strict=True, ignore=None):
    print('\nLoading model & parameters ...')
    state = torch.load(os.path.join(model_folder, 're.model'),
                                             map_location=trainer.model.device)
    if ignore is not None:
        state = {k:v for k,v in state.items() if k not in ignore}
    miss_keys, unexpection_keys = trainer.model.load_state_dict(state, strict=strict)
    print("miss_keys", miss_keys)
    print("unexpection_keys", unexpection_keys)
    # for k, v in trainer.model.state_dict().items():
    #     print("k\t",k)
        # print(v)
    return trainer
