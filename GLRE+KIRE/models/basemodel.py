...
from nnet.atloss import ATLoss
...
class BaseModel(nn.Module):
    ...
    def __init__(self, params, pembeds, loss_weight, sizes, maps, lab2ign):
        ...
        self.dist_embed_dir = EmbedLayer(num_embeddings=20, embedding_dim=params['dist_dim'],
                                        ...
                                        pretrained=None,
                                         mapping=None)
        if 'use_coref' in params and params['use_coref']:
            self.coref_pos_emb = EmbedLayer(num_embeddings=100, embedding_dim=params['type_dim'], ignore=0, dropout=0.0)
        ...
        self.adaptive_threshlod = params['adaptive_threshlod']
        if self.adaptive_threshlod:
            self.at_loss = ATLoss()
    ...
    def estimate_loss(self, pred_pairs, truth, multi_truth):
        ...
        if self.adaptive_threshlod:
            output = self.at_loss.get_label(pred_pairs, num_labels=4)
            loss = self.at_loss(pred_pairs, multi_truth)
        else:
        ...
        if self.adaptive_threshlod:
            return loss, stats, predictions, pred_pairs, multi_truth, multi_mask, truth, output
        else:
            return loss, stats, predictions, pred_pairs, multi_truth, multi_mask, truth, None