...
class RGCN(nn.Module):
    ...
    def __init__(self, in_dim, mem_dim, num_layers, relation_cnt, gpu, gcn_in_drop=0.5, gcn_out_drop=0.5):
        super().__init__()
        self.device = torch.device("cuda" if gpu != -1 else "cpu")
    ...
    def forward(self, nodes, adj):
        ...
        for batch in range(batch_size):
            denoms = []
            for i in range(self.relation_cnt):
                if adj[batch][i]._values().size(0) == 0:
                    denom = torch.zeros(nodes.size(1)).to(self.device)
                else:
                    denom = torch.sparse.sum(adj[batch][i], dim=1).to_dense()
                    # denom = torch.sum(adj[batch][i].to_dense(), dim=1)
        ...
        # sparse rgcn layer
        # eye_matrix = torch.eye(gcn_inputs.size(1)).to_sparse().cuda()
        for l in range(self.layers):
            gAxWs = []
            for j in range(self.relation_cnt):
                ...
                for batch in range(batch_size):
                    # xW = bxW[batch]
                    # AxW = torch.mm(adj[batch][j].to_dense(), xW)
                    if adj[batch][j]._values().size(0) == 0:
                        gAxW.append(torch.zeros(gcn_inputs.size(1), gcn_inputs.size(2)).to(self.device))
                    else:
                        xW = bxW[batch]  # 255 * 25
                        # eye_de = torch.sparse.mm(eye_matrix, xW)
                        # print("eye_de", eye_de.size())
                        AxW = torch.sparse.mm(adj[batch][j], xW) # - eye_de  # 255, 25
...
class SpGAT(nn.Module):
    ...
    def forward(self, nodes, adj, nodes_query, adj_edges=None):
        ...
        #     nodes_query = nodes
        # elif self.gcn_type == 'gat_wordrep':  # 以实体token表示作为查询向量
        #     pass
        # elif self.gcn_type == 'gat_relrep':  # 以
        #     pass
