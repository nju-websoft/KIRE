...
def to_official(preds, features, dataset='docred'):
    if dataset == 'docred':
        id2rel = {value: key for key, value in docred_rel2id.items()}
    elif dataset == 'dwie':
        id2rel = {value: key for key, value in dwie_rel2id.items()}
    h_idx, t_idx, title = [], [], []
    ep_dis = []
    ...
    for f in features:
        ...
        # print(f['ep_dis'])
        ep_dis += [f['ep_dis'][(ht[0], ht[1])] for ht in hts]
...
def official_evaluate(tmp, path, tag):
    ...
    return re_f1, re_p, re_r, evi_f1, re_f1_ignore_train_annotated,re_p_ignore_train_annotated, re_r, re_f1_ignore_train
