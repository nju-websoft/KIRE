...
class Trainer():
    def __init__(self, train_data, dev_data, pred_embedding, eos_index):
        ...
        self.go = eos_index
        self.eos = eos_index
        self.model = AutoEncoderModel(lstm_units, pred_embedding, eos_index, train_embeddings)
    ...
    def run(self):
        ...
        self.model.load_state_dict(torch.load('./ae_results/re.model'))
        print("load model")
        ...
        while self.train_data.epoch_counter < self.epochs:
            ...
            self.model.train()
            batch_counter += 1
            train_sents, train_sizes = self.train_data.next_batch(self.batch_size, True)
            ...
            if batch_counter % report_interval == 0:
                ...
                while self.dev_data.epoch_counter < 1:
                    dev_sents, dev_sizes = self.dev_data.next_batch(self.batch_size, False)
                ...
                if val_loss/val_sum < best_loss:
                    best_loss = val_loss/val_sum
                    torch.save(self.model.state_dict(), os.path.join(self.save_dir, 're.model'))
                    msg += '\t(saved model)'
...
class Dataset(object):
    ...
    def next_batch(self, batch_size, train=False):
        """
        ...
        if train:
            if len(matrix) > 1e7:
                last_ml = 0.01 * last_ml
            elif len(matrix) > 1e6:
                last_ml = 0.1 * last_ml

        if self.next_batch_ind >= last_ml:
            self.last_matrix_ind += 1
            if self.last_matrix_ind >= len(self.sentence_matrices):
                if train:
                    for sentences, sizes in zip(self.sentence_matrices, self.sizes):
...
def train():
    ...
    print("sentence number in train dataset = ", len(train_data))  # 18312793
    dev_data = load_binary_data('./ae_results/dev-data.npz')
    print("sentence number in dev dataset = ", len(dev_data))  # 373725
...
def test():
    sentence = 'label # House Scanning the HIM Horizon </s>'
    ...
    model = AutoEncoderModel(50, vec, eos_index)
    model = model.cuda()
...
def prepare_data():
    ...
    properties = set()
    with open('../data/DocRED/one_hop_property_triplets.nt3', 'r', encoding='utf-8') as f:
    ...
    docs = set()
    for file_path in ['../data/DocRED/train_annotated.json', '../data/DocRED/dev.json', '../data/DocRED/test.json']:
    ...
    print("sentence number=%d， len>128=%d" % (len(new_sents), len_filter_cnt))
...
def prepare_data1():
    ...
    id2word = {v:k for k,v in word2id.items()}
    new_sents = []
    for sents_file, lens_file in [('../data/DocRED/processed/dev_train_kg_subg_entity_attrs.npy', '../data/DocRED/processed/dev_train_kg_subg_entity_attr_lens1.npy'),
                                  ('../data/DocRED/processed/kg_data/dev_dev_kg_subg_entity_attrs.npy', '../data/DocRED/processed/dev_dev_kg_subg_entity_attr_lens.npy'),
                                  ('../data/DocRED/processed/kg_data/dev_test_kg_subg_entity_attrs.npy', '../data/DocRED/processed/dev_test_kg_subg_entity_attr_lens.npy')]:
        train_kg_entity_attrs = np.load(sents_file)  # (all_attr_size, max_attr_len=128)
        train_kg_entity_attr_lens = np.load(lens_file)
        for i in range(train_kg_entity_attrs.shape[0]):
            lens = train_kg_entity_attr_lens[i]
            if lens < 1:
                continue
            tokens = train_kg_entity_attrs[i][:lens]
            sents = [id2word[k] for k in tokens]
            new_sents.append(' '.join(sents))
