...
dwie_rel2id = json.load(open('./data/DWIE/rel2id.json', 'r'))
...
def read_docred(file_in, tokenizer, max_seq_length=1024, dataset='docred'):
    if dataset == 'docred':
    ...
    for did, sample in tqdm(enumerate(data), desc="Example"):
        if 'title' not in sample:
            sample['title'] = str(did)
        ...
        assert len(relations) == len(entities) * (len(entities) - 1), print(len(relations), len(entities))
    ...
    print("# of positive examples {}, {}.".format(pos_samples, pos_rela_samples))
