...
def docred_convert_examples_to_features(
    ...
    if dataset == "DocRED":
        ...
        with open(os.path.join("./data/DWIE/", "ner2id.json"), 'r') as f:
            ner_map = json.load(f)
    if dataset == "DocRED":
        ...
        distance_buckets = numpy.zeros((2500), dtype='int64')
        distance_buckets[1] = 1
        distance_buckets[2:] = 2
        distance_buckets[4:] = 3
        distance_buckets[8:] = 4
        ...
        distance_buckets[128:] = 8
        distance_buckets[256:] = 9
        distance_buckets[512:] = 10
        distance_buckets[1024:] = 11
        distance_buckets[2048:] = 12
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tok_to_sent = []
        tok_to_word = []
        # Modify code here, add subword_indexs
        ...
        for sent_idx, sent in enumerate(example.sents):
            for word_idx, word in enumerate(sent):
                ...
                #if (sent_id, word_idx) in entity_start:
                #    index = index + 1
                #    flag = True
                #if(ex_index ==42):
                #    print(index)
                ...
                #if(ex_index ==42):
                #    print(subword_indexs)
                index += len(tokens_tmp)
                #if flag:
                #    index -= 1
                #flag = False
        ...
        for j in range(len(entities)):
            for k in range(len(entities)):
                if j != k:
                    min_dis = 100
                    for n1 in entities[j]:
                        for n2 in entities[k]:
                            dis = abs(int(n1['sent_id']) - int(n2['sent_id']))
        ...
        if ex_index == 42:
            ...
            print("subword_indexs: %s" % (" ".join([str(x) for x in subword_indexs])))
        '''
        features.append(
            DocREDInputFeatures(
                input_ids=input_ids,
...
class DocREDInputFeatures(object):
    ...
    """
    # Modify code here, add subword_indexs
    def __init__(self, input_ids, attention_mask, token_type_ids, ent_mask, ent_ner, ent_pos, ent_distance, structure_mask, subword_indexs, dids, ep_dis, label=None, label_mask=None):
        ...
        self.subword_indexs = subword_indexs
        self.dids = dids
        self.ep_dis = ep_dis
