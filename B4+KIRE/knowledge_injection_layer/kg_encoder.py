...
from knowledge_injection_layer.modules import GCN, DSGCN, SpGAT
...
class Attr_encoder(nn.Module):
    def __init__(self, pembeds, kg_freeze_words, ent_hidden_dim, gpuid):
        ...
        if kgconfig.attr_encode_type == 'max':
            self.word_embed = nn.Embedding.from_pretrained(pembeds, freeze=kg_freeze_words)
            ...
            self.autoenc.load_state_dict(torch.load('./knowledge_injection_layer/ae_results/re.model', pickle_module=pickle), strict=False)
    ...
    def forward(self, entity_attrs, attrs_nums, entity_attr_lens):
        ...
        if kgconfig.attr_encode_type == 'max':
            ...
            if torch.isnan(_word_vec).any():
                print(self.word_embed.weight.data)
                print(entity_attrs)
                print(_word_vec)
                print("_word_vec")
                exit(-1)
            ...
            if kgconfig.attr_encode_lstm:  # 加了lstm 似乎没啥用
                word_vec_packed = pack_padded_sequence(word_vec, entity_attr_lens, batch_first=True, enforce_sorted=False)
                encoded_attr_outputs, (encoded_attr_hiddens, _) = self.lstm_encoder(word_vec_packed)  # batch_attr_nums， max_attr_len, -1
        ...
        if torch.isnan(encoded_attr_hiddens).any():
            print(encoded_attr_hiddens)
            print("encoded_attr_hiddens")
            exit(-1)
        ...
        output_attr_hiddens = pad_sequence(torch.split(output_attr_hiddens, entity_nums), batch_first=True)  # batch_size, entity_size, -1
        # output_attr_hiddens = F.normalize(output_attr_hiddens, dim=-1)

        output_attr_hiddens = torch.where(torch.isnan(output_attr_hiddens), torch.full_like(output_attr_hiddens, 0), output_attr_hiddens)
        if torch.isnan(output_attr_hiddens).any():
            print(_encoded_attr_hiddens)
            print(output_attr_hiddens)
            print("output_attr_hiddens")
            exit(-1)
...
class Kg_Encoder(nn.Module):
    ...
    def forward(self, entity_attrs, attrs_nums, entity_attr_lens, kg_adj, entity_doc_nodeid, kg_adj_edges=None):
        ...
        if torch.isnan(attr_hiddens).any():
            print("attr_hiddens", attr_hiddens)
            exit(-1)
