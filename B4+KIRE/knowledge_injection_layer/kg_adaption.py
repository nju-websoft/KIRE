...
class Kg_Adaption(nn.Module):
    ...
    def _forward_gate(self, doc_hidden_states, doc_reps, ent_hidden_states, attention_mask_ent):
        ...
        # print(doc_reps.size())
        # print(doc_hidden_states.size())
        # print(ent_hidden_states.size())

        gate = torch.sigmoid(self.w(torch.cat([doc_reps, doc_hidden_states, ent_hidden_states], dim=-1)))
...
class adaption_layer(nn.Module):
    def __init__(self, hidden_dim, ent_hidden_dim, kg_intermediate_dim, kg_num_attention_heads, kg_attention_probs_dropout_prob):
        ...
        self.word_self_output = SelfOutput(hidden_dim, hidden_dim, dropout_prob=kg_attention_probs_dropout_prob)

        self.ent_self_att = nn.MultiheadAttention(embed_dim=ent_hidden_dim,  # ent_hidden_dim = 100
                                                  num_heads=kg_num_attention_heads,
                                                  dropout=kg_attention_probs_dropout_prob)
        self.ent_self_output = SelfOutput(ent_hidden_dim, ent_hidden_dim, dropout_prob=kg_attention_probs_dropout_prob)
        ...
        self.final_word_self_output = SelfOutput(kg_intermediate_dim, hidden_dim, dropout_prob=kg_attention_probs_dropout_prob)
        self.final_ent_self_output = SelfOutput(kg_intermediate_dim, ent_hidden_dim, dropout_prob=kg_attention_probs_dropout_prob)
        self.kg_num_attention_heads = kg_num_attention_heads
