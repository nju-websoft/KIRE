...
parser.add_argument('--in_path', type = str, default =  "../DWIE/data")
parser.add_argument('--out_path', type = str, default = "prepro_data/dwie")
	...
	for i in range(len(ori_data)):
		...
		for j in range(len(vertexSet)):
			for k in range(len(vertexSet[j])):
				...
				assert pos1 == dl + vertexSet[j][k]['pos'][0]
				assert pos2 < 1800
		...
		item['title'] = str(i) #ori_data[i]['title']
...
init(train_annotated_file_name, rel2id, max_length = 1800, is_training = False, suffix='_train')
init(dev_file_name, rel2id, max_length = 1800, is_training = False, suffix='_dev')
init(test_file_name, rel2id, max_length = 1800, is_training = False, suffix='_test')
